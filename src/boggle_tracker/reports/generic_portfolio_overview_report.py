...
class GenericPortfolioOverviewReport(BaseReport):
    def generate(self):
        ...
        for portfolio in self.portfolios:
            report_content += self._generate_portfolio_report(
                portfolio, currency_summaries
            )
    ...
    def _generate_portfolio_report(self, portfolio, currency_summaries):
        ...
        merged_holdings = self._merge_holdings(portfolio.holdings)

        total_market_value = 0
        total_cost_basis = 0

        for symbol, holding_data in merged_holdings.items():
        ...
        report_content += (
            f"Total Unrealized Gain/Loss: ${total_unrealized_gain_loss:.2f}\n\n"
        )
    ...
    def _merge_holdings(self, holdings):
        merged = defaultdict(lambda: {"market_value": 0, "cost_basis": 0})
        for holding in holdings:
            try:
                ...
                if current_price is None or quantity is None:
                    print(
                        f"Warning: Missing crucial data for holding {symbol}. Skipping this holding."
                    )
                    continue
                ...
                merged[symbol]["market_value"] += market_value
                merged[symbol]["cost_basis"] += cost_basis
    ...
    def _generate_currency_summaries(self, currency_summaries):
        ...
        for currency, summary in currency_summaries.items():
            ...
            report_content += (
                f"Total Unrealized Gain/Loss: ${unrealized_gain_loss:.2f}\n\n"
            )
        return report_content
